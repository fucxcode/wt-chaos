/**
 * fallback version of "lodash-wrapper.ts"
 * 
 * because in typescript 3.1+, the type definition of "lodash" reported a TS2321 error
 * there is no error when compiling (tsc) but failed in VSCode and "ts-node"
 * hence we use "require" to load "lodash" to skipping the type definition and wait for typescript 3.3 release with this bug fixed
 * 
 * ref: https://github.com/Microsoft/TypeScript/issues/21592
 */
...
export function keyBy<T>(collection: ArrayLike<T> | null | undefined, iteratee: any): Dictionary<T> {
...
export function forIn<T>(object: T, iteratee?: ObjectIterator<T, any>): T {
    return _.forIn(object, iteratee);
}
...
export function forOwn<T>(object: T | null | undefined, iteratee?: ObjectIterator<T, any>): T | null | undefined {
    return _.forOwn(object, iteratee);
}
